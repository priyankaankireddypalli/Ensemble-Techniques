...
tumor = pd.read_csv("C:\\Users\\WIN10\\Desktop\\LEARNING\\DS\\Tumor_Ensemble.csv")
tumor.describe()
tumor.columns
tumor.drop(['id'],axis=1,inplace=True)
...
import pandas as pd
import numpy as np
tumor = pd.read_csv("C:\\Users\\rammo\\Downloads\\DS\\Ensemble Techniques\\Tumor_Ensemble.csv")
tumor.describe()
tumor.columns
tumor.drop(['id'],axis=1,inplace=True)
# Creating Dummy variables
# n-1 dummy variables will be created for n categories
tumor = pd.get_dummies(tumor, columns = ['diagnosis'], drop_first = True)
# Input and Output Split
predictors = tumor.loc[:, tumor.columns!='diagnosis_M']
target = tumor['diagnosis_M']
# Splitting the data into train and test
from sklearn.model_selection import train_test_split
x_train, x_test, y_train, y_test = train_test_split(predictors, target, test_size = 0.2, random_state=0)
