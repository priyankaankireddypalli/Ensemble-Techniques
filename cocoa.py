...
cocoa = pd.read_excel("C:\\Users\\WIN10\\Desktop\\LEARNING\\DS\\Coca_Rating_Ensemble.xlsx")
cocoa.describe()
cocoa.drop(['Bean_Type', 'Origin'],axis=1,inplace=True)
cocoa.columns
cocoa.info()
cocoa.Cocoa_Percent = cocoa.Cocoa_Percent.astype('int64')
cocoa.Rating = cocoa.Rating.astype('int64')
...
cocoa = pd.get_dummies(cocoa, columns = ['Company', 'Name','Company_Location'], drop_first = True)
# Input and Output Split
predictors = cocoa.loc[:, cocoa.columns!='Rating']
target = cocoa['Rating']
...
boost_clf2 = GradientBoostingClassifier(learning_rate = 0.02, n_estimators = 50, max_depth = 1)
